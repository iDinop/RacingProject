...
void AVehicleSystemBase::Tick(float DeltaTime)
{
	...
	// Rigid body sleep
	if( !VehicleMesh->IsAnyRigidBodyAwake() )
	{
		...
		// If we have throttle, don't allow physics sleep
		if( InputsForPhysicsThread.Throttle > 0.01f && InputsForPhysicsThread.Torque > 0.01f )
		{
			// Apply a small acceleration, or chaos will just re-sleep the rigid body
			FVector Boop = VehicleMesh->GetForwardVector() * 2500.0f;
			if(InputsForPhysicsThread.ReverseTorque) Boop *= -1.0f;
			VehicleMesh->AddForce(Boop, NAME_None, true);
		}
	}
}
...
void AVehicleSystemBase::AVS_PhysicsTick(float ChaosDelta, const FVehiclePhysicsPhysicsInput* PhysicsInput, FVehiclePhysicsPhysicsOutput& PhysicsOutput)
{
	...
	for( int32 WIndex = 0; WIndex < Wheels.Num(); ++WIndex )
	{
		...
		if(TraceHit)
		{
			...
			const float InterpSpeedLong = FMath::Clamp(FMath::Abs(WheelVelocityLocalM.X) / 0.010f * ChaosDelta, 0.0f, 1.0f);
